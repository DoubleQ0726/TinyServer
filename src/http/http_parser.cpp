...
void on_request_method(void *data, const char *at, size_t length)
{
    ...
    HttpMethod hm = CharsToHttpMethod(at);
    if (hm == HttpMethod::HTTP_METHOD_INVALID)
    {
        TINY_LOG_WARN(logger) << "invalid http request method: " << std::string(at, length);
        parser->setError(1000);
        return;
    }
    parser->getData()->setMethod(hm);
}
...
void on_request_version(void *data, const char *at, size_t length)
{
    ...
    {
        TINY_LOG_WARN(logger) << "invalid http request version: " << std::string(at,length);
...
{
    HttpResponseParser* parser = static_cast<HttpResponseParser*>(data);
    HttpStatus hs = (HttpStatus)(atoi(at));
    parser->getData()->setStatus(hs);
}
...
void on_response_version(void *data, const char *at, size_t length)
{
    ...
    if (strncmp(at, "HTTP/1.0", length) == 0)
        version = 0x11;
    else if (strncmp(at, "HTTP/1.0", length))
        version = 0x10;
    else
    {
        TINY_LOG_WARN(logger) << "invalid http response version: " << std::string(at,length);
        parser->setError(1001);
        return;
    }
...
void on_reponse_last_chunk(void *data, const char *at, size_t length)
{
    ...
    : m_error(0)
{
    ...
    m_parser.last_chunk = on_reponse_last_chunk;
