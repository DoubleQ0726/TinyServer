...
Ref<Address> Address::Create(const sockaddr* addr, socklen_t addrlen)
{
    if (addr == nullptr)
        return nullptr;
    Ref<Address> result;
...
bool Address::Lookup(std::vector<Ref<Address>>& result, const std::string& host, int family, 
    int type, int protocol)
{
    ...
    if (error)
    {
        TINY_LOG_ERROR(logger) << "Address::Lookup getaddress(" << host << ", " << family
...
Ref<Address> Address::LookupAny(const std::string& host, int family, 
    int type, int protocol )
{
    std::vector<Ref<Address>> result;
    if (Lookup(result, host, family, type, protocol))
        return result[0];
    return nullptr;
}

Ref<IPAddress> Address::LookupIPAddress(const std::string& host, int family, 
    int type, int protocol)
{
    std::vector<Ref<Address>> result;
    if (Lookup(result, host, family, type, protocol)) 
    {
        ...
        {
            Ref<IPAddress> res = std::dynamic_pointer_cast<IPAddress>(item);
            if (res)
                return res;
        }
    }
...
bool Address::GetInterfaceAddress(std::multimap<std::string, std::pair<Ref<Address>, uint32_t>>& result,
        int family)
{
    struct ifaddrs *results, *next;
    if (getifaddrs(&results) == -1)
    {
        TINY_LOG_ERROR(logger) << "Address::GetInterfaceAddress getifaddrs " << "err = " << errno << "errstr = " << strerror(errno);
        return false;
    }
    try
    {
        for (next = results; next; next = next->ifa_next)
        {
            ...
            case AF_INET6:
                {
                    addr = Address::Create(next->ifa_addr, sizeof(sockaddr_in6));
    ...
    {
        TINY_LOG_ERROR(logger) << "Address::GetInterfaceAddress exception";
        freeifaddrs(results);
        return false;
    }
    freeifaddrs(results);
...
bool Address::GetInterfaceAddress(std::vector<std::pair<Ref<Address>, uint32_t>>& result, const std::string& ifcae, int family)
{
    if (ifcae.empty() || ifcae == "*")
    {
        ...
        {
            result.push_back(std::make_pair(Ref<IPv4Address>(new IPv4Address()), 0u));
        }
        if (family == AF_INET6 || family == AF_UNSPEC)
        {
            result.push_back(std::make_pair(Ref<IPv6Address>(new IPv6Address()), 0u));
        }
        return true;
    }
...
//主机名到地址解析
Ref<IPAddress> IPAddress::Create(const char* address, uint16_t port)
{
    ...
    if (error)
    {
        TINY_LOG_ERROR(logger) << "IPAddress::Create(" << address << ", " << port
            << ") error = " << error << " errno = " << errno << " errstr = " << strerror(errno);
        return nullptr; 
    }
    try
    {
        Ref<IPAddress> result = std::dynamic_pointer_cast<IPAddress>
...
Ref<IPv4Address> IPv4Address::Create(const char* address, uint16_t port)
{
    Ref<IPv4Address> res(new IPv4Address);
    res->m_addr.sin_port = byteswapOnLittleEndian(port);
    int result = inet_pton(AF_INET, address, &res->m_addr.sin_addr);
    if (result <= 0)
    {
...
Ref<IPAddress> IPv4Address::broadcastAddress(uint32_t prefix_len)
{
    if (prefix_len > 32)
        return nullptr;
    sockaddr_in b_addr(m_addr);
    b_addr.sin_addr.s_addr |= byteswapOnLittleEndian(CreateMask<uint32_t>(prefix_len));
    return Ref<IPv4Address>(new IPv4Address(b_addr)); 
}

Ref<IPAddress> IPv4Address::netWordAddress(uint32_t prefix_len)
{
    if (prefix_len > 32)
        return nullptr;
    sockaddr_in n_addr(m_addr);
    n_addr.sin_addr.s_addr &= byteswapOnLittleEndian(CreateMask<uint32_t>(prefix_len));
    return Ref<IPv4Address>(new IPv4Address(n_addr));
}

Ref<IPAddress> IPv4Address::subnetMask(uint32_t prefix_len)
{
    sockaddr_in subnet;
    memset(&subnet, 0,sizeof(subnet));
    subnet.sin_family = AF_INET;
    subnet.sin_addr.s_addr = ~byteswapOnLittleEndian(CreateMask<uint32_t>(prefix_len));
    return Ref<IPv4Address>(new IPv4Address(subnet));
}
...
Ref<IPv6Address> IPv6Address::Create(const char* address, uint16_t port)
{
    Ref<IPv6Address> res(new IPv6Address);
...
Ref<IPAddress> IPv6Address::broadcastAddress(uint32_t prefix_len)
{
    ...
    return Ref<IPv6Address>(new IPv6Address(b_addr));
}

Ref<IPAddress> IPv6Address::netWordAddress(uint32_t prefix_len)
{
    ...
    //     b_addr.sin6_addr.s6_addr[i] = 0xff;
    // }
    return Ref<IPv6Address>(new IPv6Address(n_addr));
}

Ref<IPAddress> IPv6Address::subnetMask(uint32_t prefix_len)
{
    sockaddr_in6 subnet;
    memset(&subnet, 0,sizeof(subnet));
    ...
    return Ref<IPv6Address>(new IPv6Address(subnet));
}
...
UnixAddress::UnixAddress()
{
    ...
    //实际所用空间长度
    m_length = offsetof(sockaddr_un, sockaddr_un::sun_path) + MAX_PATH_LEN;
}
...
UnixAddress::UnixAddress(const std::string& path)
{
    ...
    m_length += offsetof(sockaddr_un, sockaddr_un::sun_path);
}
...
{
    if (m_length > offsetof(sockaddr_un, sockaddr_un::sun_path) && m_addr.sun_path[0] == '\0')
    {
