...
Ref<Uri> Uri::Create(const std::string& uri_str)
{
    Ref<Uri> uri(new Uri);
    int cs = 0;
    const char* mark = 0;
...
#line 144 "uri.rl"
    ...
    else if (cs >= uri_parser_first_final)  //>= 表示解析完成
        return uri;
    return nullptr;
}
...
Ref<Address> Uri::createAddress() const
{
    auto addr = Address::LookupIPAddress(m_host);
    if (addr)
